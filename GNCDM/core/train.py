...
def apply_mnar_mask_to_data(log_data: np.ndarray, Q_new: np.ndarray, 
                            new_item_indices: np.ndarray, gamma: float = 1.0, tau: float = 0.5):
    """
...
class AsymmetricHybridDataLoader:
    ...
    def __init__(self, old_loader: DataLoader, new_loader: DataLoader, 
                 old_ratio: float = 0.7, device=None):
...
def calculate_tmd(theta_old: torch.Tensor, theta_new: torch.Tensor, K_old: int) -> float:
    """
    Calculate Trait Manifold Drift (TMD) between old and new theta vectors.
    ...
    Returns:
        tmd: float, normalized mean drift across all users
    """
    # Extract the first K_old dimensions from theta_new
    theta_new_old_dim = theta_new[:, :K_old]
    
    # Compute per-user L2 norm of difference
    per_user_norm = torch.norm(theta_old - theta_new_old_dim, dim=1)
    
    # Normalize by sqrt(K_old) and take mean
...
def eval_incremental(model_old, model_dynamic, data: pd.DataFrame, 
                     original_n_item: int, original_n_know: int, batch_size: int = 16):
    """
    Decoupled evaluation for incremental learning.
    
    Separately evaluates performance on old items and new items,
    and calculates Trait Manifold Drift (TMD).
    ...
    Returns:
        eval_result: dict, containing AUC_old, RMSE_old, AUC_new, RMSE_new, and TMD
    """
    model_old.eval()
    model_dynamic.eval()
    ...
    # Calculate TMD
    print('=== Trait Manifold Drift (TMD) ===')
    with torch.no_grad():
        ...
        tmd = calculate_tmd(theta_old, theta_new, original_n_know)
        print(f'TMD = {tmd:.6f}')
        eval_result['tmd'] = tmd
...
def train_incremental(model_old, model_dynamic, train_data_old: pd.DataFrame, 
                      train_data_new: pd.DataFrame, valid_data: pd.DataFrame,
                      original_n_know: int, V_old: int, V_new: int,
                      batch_size: int = 64, lr: float = 1e-3, n_epoch: int = 50):
    """
