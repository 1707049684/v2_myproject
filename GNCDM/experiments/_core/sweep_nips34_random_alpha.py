# -*- coding: utf-8 -*-
"""Alpha sweep on nips34 (Eedi NeurIPS 2020 Task 3&4) random_split — 选 Ours 最优 alpha。

与 sweep_a0910/junyi/math1 同口径：buf 预测，每个 alpha 训 Base + Ours(Dynamic DNA) +
Ours(LoRA)，**在 valid 上按 DNA mean(valid AUC_old, AUC_new) 选 alpha、test 上报数**。
只用 G-NCDM，不需要 avalanche。

nips34 数据：GNCDM/data/nips34_*（4918 user × 948 item × 57 KC，稀疏 Q≈1.01 KC/题，
score=IsCorrect 原生 0/1）。新题占比 35.6%≈math1 的 36% → 期望最优 alpha≈0.20，本扫确认。
交互量大(1.38M)，10 个 alpha × (Base+DNA+LoRA) × 25ep —— 建议 GPU。产物：
incremental_result/alpha_sweep_nips34_random_split.csv
"""
...
def load():
    Q = np.load(os.path.join(DATA, f"{PREFIX}_Q_matrix.npy"))
    n_item_total, n_know_total = Q.shape
    nc = auto_new_concepts(Q, 0.34)
    df_tr = pd.read_csv(os.path.join(DATA, f"{PREFIX}_train_0.8_0.1_0.1.csv"))
    df_va = pd.read_csv(os.path.join(DATA, f"{PREFIX}_valid_0.8_0.1_0.1.csv"))
    df_te = pd.read_csv(os.path.join(DATA, f"{PREFIX}_test_0.8_0.1_0.1.csv"))
...
def main():
    ...
    if device.type == "cpu":
        print("⚠️ nips34 交互量大(1.38M)，10 alpha 全扫 CPU 偏慢，建议 GPU 服务器。")
    c = load()
    print(
    ...
    out = os.path.join(R.SAVE_DIR, "alpha_sweep_nips34_random_split.csv")
