# -*- coding: utf-8 -*-
"""Alpha sweep on XES3G5M（子采样 ~1M 交互）random_split — 选 Ours 最优 alpha。

与 sweep_a0910/junyi/math1/nips34 同口径：buf 预测，每个 alpha 训 Base + Ours(Dynamic DNA) +
Ours(LoRA)，**在 valid 上按 DNA mean(valid AUC_old, AUC_new) 选 alpha、test 上报数**。
只用 G-NCDM，不需要 avalanche。

xes 数据：GNCDM/data/xes_*（3402 user × 7056 item × 837 KC，稀疏 Q≈1.16 KC/题，
score=responses 0/1，pos_rate≈0.79）。新概念占比 82%>a0910 67% → 期望最优 alpha≈0.1，本扫确认。
交互量 ~1M、KC 837 —— 建议 GPU。产物：
incremental_result/alpha_sweep_xes_random_split.csv
"""
...
def main():
    ...
    if device.type == "cpu":
        print("⚠️ xes 交互量 ~1M、KC 837，10 alpha 全扫 CPU 偏慢，建议 GPU 服务器。")
    c = load()
    print(
    ...
    out = os.path.join(R.SAVE_DIR, "alpha_sweep_xes_random_split.csv")
