...
### ׷�ӣ�alpha ������ͼ��Math1 / junyi / a0910 random_split��
- �ű���`GNCDM/plot/plot_alpha_sensitivity_random.py`�������� `alpha_sweep_*_random_split.csv`����
- ���᣺ѡ���׼ `sel_DNA_validACC` + ���� `DNA_te_ACCnew`�����Ǳ���ʵ��ѡ�� ����Math1=0.20��junyi/a0910=0.10����
- ���`incremental_result/alpha_sensitivity_random_split.{png,pdf,svg}`��˳������ a0910 CSV �ļ���β��ո�
- **��ǰ״̬**���Ķ�δ�ύ��

### ׷�ӣ�Math1 G-NCDM �� UMAP ��ͼ��Fig.10 ���
- �ű���`GNCDM/plot/plot_umap_math1_gncdm.py`��ȫ�� 20 �� G-NCDM����=0.8��15 ep��`diagnose_theta` �� UMAP �� RdBu_r �� score rate ��ɫ����
- ���`incremental_result/umap_math1_gncdm.{png,pdf,svg}`������ `umap_cache_math1/{theta_gncdm,score_rate,umap_xy_gncdm}.npy`��
- �۲죺ˮƽ���������������÷����ݶȣ������� Fig.10(d) ��̬һ�¡���һ���ɼ� CLEAN-Full ������塣
- **��ǰ״̬**���Ķ�δ�ύ��

### ׷�ӣ�UMAP ˫��� G-NCDM vs CLEAN-Full��Math1��
- ��չ `plot_umap_math1_gncdm.py`����� (a) ȫ�� G-NCDM��(b) Base+Dynamic DNA����=0.20��`mask_agg_old`����
- ���`incremental_result/umap_math1_gncdm_vs_clean_full.{png,pdf,svg}`������ `theta_clean_full.npy` / `umap_xy_clean_full.npy`��
- **��ǰ״̬**���Ķ�δ�ύ��

### ׷�ӣ�UMAP CLEAN-Full vs CLEAN-LoRA��Math1��
- `plot_umap_math1_gncdm.py` Ĭ�� `--compare full_lora`������ `umap_math1_clean_full_vs_lora.{png,pdf,svg}`��
- CLEAN-Full ���û��棻CLEAN-LoRA ��ѵ Base+Ours(LoRA) ������ `theta_clean_lora.npy`��
- **��ǰ״̬**���Ķ�δ�ύ��

### 追加：旧题预测分布消融图（对应 OrthoMask / FrozenBias）
- 脚本：`GNCDM/plot/plot_old_pred_dist_ablation.py`（与 `run_ablation_dna_a0910_user_split.py` 同协议：user_split support/query）。
- 方法：Base / CLEAN-Full / w/o OrthoMask / w/o FrozenBias / w/o OCM(=Ours-Ablated)。
- Math1 本地已跑：CLEAN-Full 与 Base 的 `mean|Δŷ|=0`（逐位相同）；三消融全面偏移。
- 产物：`old_pred_dist_ablation_math1_user_split.{png,csv}`；pdf/svg 在 `additonal/`。
- a0910（对齐表 `ablation_a0910_user_split.csv`）需 GPU：`python plot_old_pred_dist_ablation.py --dataset a0910`。
- **当前状态**：改动未提交。

### 追加：旧题预测 AlignedUMAP（Math1 random_split 消融）
- 脚本：`plot_aligned_umap_math1_pred_ablation.py`；嵌入对象是 `ŷ_old ∈ R^{M_old}`（非 θ_old，否则消融看不出差别）。
- 口径：Math1 random_split、α=0.20、`forward_using_buf`。
- CLEAN-Full：mean |y1-y0|_2=0；w/o OrthoMask≈0.78；w/o FrozenBias≈0.17；w/o OCM≈0.76。
- 产物：`aligned_umap_math1_pred_ablation_grid.png` + 各方法 `*_pred_drift.png`；pdf/svg 在 `additonal/`。
- **当前状态**：改动未提交。

### ׷�ӣ������Լ���Ϊ Balanced_ACC���޳� CLEAN-LoRA / Full-Replay
- ���յ㣺`Balanced_ACC = 0.7*ACC_old + 0.3*ACC_new`���� `_stat_acc_clone` �������֣���
- ������ roster ֻѵ/�ȣ�Anchor + EWC/DER++/C-LoRA/X-DER/C-LoRA-GNCDM/ICD + CLEAN-Full��
- `CLEAN-LoRA`��`Full-Replay` ������ seed trial����������� `all_methods_*.csv`��
- ���£�`statistics.py`��`run_statistical_trials.py`��`docs/statistical_significance.md`����ز��ԣ�20 passed����
- **��ǰ״̬**���Ķ�δ�ύ��

### ׷�ӣ��������� random �����Խ������ Balanced_ACC �ط���
- Դ��WPS `math1_random.tar.gz` / `junyi_random.tar.gz` / `a0910_random.tar.gz` �� `incremental_result/significance_trials/*/`��
- ԭ�ܿھ���`Balanced_AUC` + �ȽϺ� CLEAN-LoRA��Full-Replay ��ѵ��δ����顣
- δ��ѵ������ `per_seed_results.csv` ���¿ھ��ط�����`Balanced_ACC`���ų� CLEAN-LoRA/Full-Replay���� `*/acc_reanalysis/`��
- Ҫ�㣺junyi/a0910 �� CLEAN-Full �� 6 ���� ACC ȫʤ��5/5����math1 ��Ӯ EWC/C-LoRA/C-LoRA-GNCDM/ICD���� DER++/X-DER �Ը����ƽ��5 seed �� exploratory_underpowered��

### ׷�ӣ��� Balanced_ACC ���ߺϲ�����������
- ��ʽ��`0.7*ACC_old + 0.3*ACC_new`��Ŀ�� CLEAN-Full������ 6 ������ LoRA/oracle����
- �ϲ����棺`incremental_result/significance_trials/balanced_acc_combined/formal_significance_acc_report.md`��

### ׷�ӣ����������Կھ�����ѵ���� LoRA/Oracle�����鲻��
